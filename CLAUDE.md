...
- `delete_gemini_project(project)` / `delete_gemini_session(project, rel)` / `delete_gemini_memory(project, rel)` — same, under `~/.gemini/tmp/<id>/` matched by `.project_root`. (Codex/OpenCode: no delete/migrate yet.)
...
**Known gaps:** Gemini migrate and Codex/OpenCode delete/migrate are NOT implemented yet. (OpenCode empty-project enumeration via the `project` table IS done — `opencode_empty_projects`.)
...
- **Remaining migrate/delete + project gaps for the other CLIs** — see "First-class projects, migration & deletion". **DONE:** delete (session/memory/project) for Claude + Gemini; migrate (project/session/memory) for Claude; OpenCode empty-project enumeration. **REMAINING** (schemas below were already inspected on real data, 2026-06; findings recorded so the next pass can start from the design, not discovery):
  ...
  **② Codex / OpenCode delete + migrate** (SQLite — higher risk; **writing a live CLI DB is an explicit exception to the "read in place" rule, so gate it hard**):
  ...
  - **DELETE = archive, NOT hard-delete** (recommended; not yet built). Set `threads.archived = 1` (+ `archived_at`) / `session.time_archived = <now ms>` — a single-row `UPDATE`: no cascade, no rollout-file deletion, **reversible**, and it's each CLI's OWN hide mechanism, so Termory's scan (already filters `archived = 0` / `time_archived IS NULL`) AND the CLI's resume both stop showing it. Hard `DELETE` is riskier (Codex's `DELETE FROM threads` is test-only in upstream; OpenCode hard-deletes via the projectors with FK cascade, needing `PRAGMA foreign_keys = ON`, plus deleting the Codex rollout JSONL) — do it only if the user explicitly wants true removal.
  - **MIGRATE = `UPDATE` the recorded cwd** (`threads.cwd` / `session.directory`; OpenCode also `project.worktree` / `project_id`). Lower value: resume-by-id already works across renames (global DB keyed by id), so migrate here is a **regrouping** convenience, not a "records lost" fix.
  - **Safety (required):** open read-write with a `busy_timeout`; if the DB is locked (CLI running) → error toast — the delete confirm's `menu.exitCliHint` ("quit the running CLI first") already primes the user. Back up the `.db` file first; wrap writes in a transaction. Mirror the existing pattern: `{archive,migrate}_{codex,opencode}_session_in(db, …)` env-free fns + IPC + a confirm flow that still sends `(project, rel)` / id (never a raw path).
