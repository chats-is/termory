...
- **Claude-only is a hard limit, not a gap.** Codex's live state lives in its app-server daemon, in memory, reachable only over an IPC socket; its on-disk files carry no status column, and rollout mtime reads identical for an idle-but-running session and a closed one. Gemini and OpenCode write no such file either.
