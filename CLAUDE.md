...
**UI flow (`ProvidersPage.tsx` + `CodexFollowDialog.tsx`).** Only an official↔custom transition changes the bucket (custom→custom stays `termory`), so the prompt fires ONLY then (`effectiveActiveId === null` ⇒ official→custom for `setAsDefault`/`activateGateway`; `!== null` ⇒ custom→official for `setOfficialAsDefault`). `maybePromptThenActivate` fetches `recent_codex_projects(0)` (limit 0 = no cap, newest-first, dialog scrolls), filters to projects with a session whose provider ≠ target; if none, activates directly with no prompt. On confirm the order is **activate first → toast → then migrate → toast** (activation returns a bool; migration only runs if it landed). The dialog uses the stock shadcn `Dialog`; project rows are raw `<button>` toggles with a `Check` icon (no shadcn Checkbox exists; raw rows are the project convention) and default UNCHECKED. IPC: `recent_codex_projects(limit)` + `follow_codex_sessions(projects, target_provider_id)` (lib.rs). Tests: `codex_follow::tests::*`.
...
- ~~Frontend test baseline~~ — done. 225 Vitest tests across 20 files.
