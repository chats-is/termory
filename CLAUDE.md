...
**Menu shape** — "Open", then up to 5 recent session titles, then one submenu per CLI (the active choice shown inline on the first-level row; submenu lists Official + the user's providers with the active one checkmarked):
...
**Known minor gaps (by design for a quick-switcher):** the tray doesn't run the Providers page's `ensureCliInstalled` gate; it doesn't expose OpenCode's multi-slot enable/disable or delete; failures are `log::error!`-only (no toast). The **provider** checkmarks/inline titles refresh only on Termory-initiated changes, so an external switch (cc-switch / manual edit) can leave them stale until the next Termory action. The **recent-sessions** list does refresh on the filesystem watcher (and every `scan_all_sessions`), so it stays current with on-disk session changes.
