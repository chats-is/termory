...
- **Static row labels are localized** (Open / Official / Exit). The tray is Rust (built in `setup()` before the frontend loads, so it can't read the i18n dicts), so the FRONTEND pushes the translated strings: an `App.tsx` effect (deps `[t]`) calls the **`set_tray_labels(open, official, exit, fiveHour, weekly, monthly, newSession, chooseFolder)`** IPC on language load/change → `tray::set_labels` stores them in a `static TRAY_LABELS: Mutex<Option<TrayLabels>>` (English default) → `rebuild_menu`. CLI / provider names stay untranslated (brand / user data). Keeps i18n single-sourced in the frontend dicts (no Rust translation table to drift). `tray.open` / `tray.official` / `tray.exit` / `tray.newSession` / `tray.chooseFolder` keys (+ the quota row reuses `providers.quotaFiveHourShort` / `providers.quotaSevenDayShort` / `providers.quotaMonthlyShort` (30_day → "Monthly")).
