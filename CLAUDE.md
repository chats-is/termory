...
Current Tauri IPC commands (37), called from the frontend with `invoke(...)`:
...
- **Backend** (`sessions.rs`, `*_in` inner fns for env-free tests): `scan_project_folders` / `claude_project_folders_in` / `gemini_project_folders_in` / `add_record_projects`; `migrate_claude_{project,session,memory}_in`; `delete_claude_{project,session,memory}_in`; `delete_gemini_{project,record}_in`; helpers `is_safe_rel`, `gemini_project_dirs`, `resolve_new_claude_dir`, `rewrite_jsonl_cwd`, `copy_tree_rewrite_cwd`. IPC wrappers in `lib.rs`.
- **Frontend:** `src/lib/migrate.ts` (`runClaudeMigration`, `runRecordDelete` — confirm → invoke → toast → success-only local callback); `src/lib/records.ts` (`projectKey`, `projectDirOf`, `recordUnderFolder`, `remappedPath`, `reconcileTombstones`, `withProject`); `recordRel` in `session-utils.ts`; `App.tsx` (`projects` state, `applyScanResult(ScanResult)`, `sourceGroups` rendered from `projects`, `removeRecordsLocally` / `remapRecordsLocally` / `projectRecordMatch` / `projectDataMatch`); per-row menu in `ListItemMenu.tsx`. i18n keys under `menu.migrate*` / `menu.delete*`.

**Tests:** `sessions::tests::{claude_project_folders_in, gemini_project_folders_in, add_record_projects, is_safe_rel, migrate_claude_*, delete_claude_*, delete_gemini_*}` + `records.test.ts` + `session-utils.test.ts` (`recordRel`) + `ListItemMenu.test.tsx` (per-entry wiring, args = `{project, rel}`, cancel paths). Whole-project migration was verified end-to-end on real copilot.is → chats.is data (39/39 files moved, `cwd` rewritten, old dir removed).

**Known gaps:** Gemini migrate and Codex/OpenCode **migrate** are NOT implemented yet. Codex/OpenCode **delete** IS done now (session/project for both, + Codex memory; real delete, sqlite rows by id/cwd + Codex rollout files — see the `delete_codex_*` / `delete_opencode_*` IPC bullets above). (OpenCode empty-project enumeration via the `project` table IS done — `opencode_empty_projects`.)
...
- **Remaining migrate/delete + project gaps for the other CLIs** — see "First-class projects, migration & deletion". **DONE:** delete (session/memory/project) for Claude + Gemini; delete for Codex (session/project/memory) + OpenCode (session/project) — see ② below; migrate (project/session/memory) for Claude; OpenCode empty-project enumeration. **REMAINING:** Gemini migrate + Codex/OpenCode **migrate** (schemas below were already inspected on real data, 2026-06; findings recorded so the next pass can start from the design, not discovery):
  ...
  **② Codex / OpenCode delete + migrate** (SQLite — higher risk; **writing a live CLI DB is an explicit exception to the "read in place" rule, so gate it hard**):
  ...
  - **MIGRATE — still TODO.** `UPDATE` the recorded cwd (`threads.cwd` / `session.directory`; OpenCode also `project.worktree` / `project_id`). Lower value: resume-by-id already works across renames (global DB keyed by id), so migrate here is a **regrouping** convenience, not a "records lost" fix.
