...
2. **Tokens** (`TokensChart.tsx`) — ONE stacked-bar chart with a **Type / Model** toggle (top-left, next to the "Tokens" title; state `TokensGroupBy = "type" | "model"`, controlled by StatsPage). Both breakdowns sum to the SAME per-day total (the locked cross-consistency rule), so the Y-axis domain (`niceMax(max daily.total)`) never jumps on toggle:
   ...
   - **Model** — per-day tokens split by model (top-6 by window tokens stacked, the rest + Unknown fold into "Others") + a legend (color dot · `displayModelName` · "{in} in · {out} out" · share%). Colors are PROVIDER-FAMILY ramps: every Claude model is a clay shade, Gemini a blue shade, OpenAI a teal shade (`--stat-<provider>-N` in globals.css; step 0 = the BOLD brand anchor for the most-used model, then LIGHTER by within-provider usage rank — so a Claude model never looks "custom"). Unknown-provider ids (custom / gateway) fall to a neutral pool (`EXTRA_MODEL_COLORS`, every hue ≥ ΔE 40 from all three brand anchors). Color tracks each model's ALL-TIME rank (StatsPage's `globalModelRank`), so a source/range filter change never repaints the survivors.
