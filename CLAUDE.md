...
Current Tauri IPC commands (35), called from the frontend with `invoke(...)`:
...
- **Remaining migrate/delete + project gaps for the other CLIs** — see "First-class projects, migration & deletion". **Delete** is done for Claude + Gemini (file-based dirs); **migrate** is Claude-only.
  - **Gemini migrate** — keyed by a per-project **hash dir** `~/.gemini/tmp/<id>/`; migration must compute Gemini's project-id hash for the new path (find the algorithm in `gemini-cli` source) + rewrite `.project_root`. Medium effort (hash derivation is the risk).
  - **Codex / OpenCode delete + migrate** — sessions live in **SQLite** (`~/.codex/state_5.sqlite` `threads`, `~/.local/share/opencode/opencode.db`). Delete = `DELETE` the row(s) (OpenCode cascades message/part); migrate = `UPDATE` the recorded `cwd` / `directory`. Higher risk: writing the CLI's live DB (violates "read in place"), locks while the CLI runs, schema drift. Needs backups + a "DB busy" guard. Resume-by-id already works across renames, so migrate here is a **regrouping** convenience, not a "records lost" fix — low priority.
