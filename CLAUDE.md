...
**Safety / rollback.** Backs up `state_5.sqlite` (best-effort, no WAL) before writing; opens RW with a 1.5s `busy_timeout` and surfaces a clean "database is locked — quit Codex" error (no write) when Codex holds the lock; per-file rewrites are atomic and only successfully-rewritten files get their table row updated. Rollout files are NOT individually backed up (single-field change, atomic write).
