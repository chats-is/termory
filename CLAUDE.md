...
**Windows paths**: `current_paths()` uses `pick_windows_claude_dir(%LOCALAPPDATA%, threep)` — exact `Claude`/`Claude-3p` fast path, else a single shallow `read_dir` scan for `Claude*`/`*-3p` variants (`windows_claude_dir_matches`, mirrors cc-switch). NOTE: the Windows path/runtime is **untested on real hardware** (logic + CI compile only); macOS is end-to-end verified (the app accepts the profile, enters 3P gateway mode, routes to the gateway — confirmed in Claude Desktop's own log).
...
- ~~Frontend test baseline~~ — done. 315 Vitest tests across 27 files.
