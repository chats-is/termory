...
**The settle drain must NOT swallow install OR credential events (LOCKED).** After each burst the watcher drains ~300 ms (`SETTLE_WINDOW`), because reading the session SQLite files touches their `-wal`/`-shm` sidecars and FSEvents reports those straight back. A package manager installs by deleting the old binary and writing the new one, and that write lands inside the drain window — so bin-dir events (`event_touches_install`) must set `install_settled_late` rather than being dropped, and the watcher re-probes after the drain. The re-probe reads only, so it cannot feed itself new events; session rescans are NOT re-run.

**Credential events are carried over the same way, and the stakes are higher**: on macOS the only trace a LOGOUT leaves is a lock under the config dir, it arrives whenever the user happened to type `/logout`, and a rescan is settling whenever a CLI has been writing session files — which is what it was doing a moment earlier. Swallowed, that signal never comes again and the tray keeps a signed-out account's numbers. They are re-dispatched through the SAME `route_credential_change` the in-burst branch uses, gate included. The identity signal is deliberately NOT carried over: `.claude.json` has 158 writers, so another lands within moments and re-runs `route_identity_change` — both halves of it, the logout clear included.
...
- **The watcher is the main path.** Claude's credential lives in the Keychain and emits nothing of its own, but the LOCKS it takes to touch that credential do — see Claude's four signal files under the tray's quota rules. `.claude.json` is routed SEPARATELY from the rest because it is not a credential.
- **One shared route, `watcher::route_credential_change`** — used by the in-burst branch AND the post-settle catch-up, so when a credential change landed cannot change what it means. It drives BOTH consumers: the quota refresh and this sync.
- **`.claude.json` must NOT join the credential route (LOCKED).** That map also feeds `force_quota_refresh`, which bypasses the quota's own floor, and `.claude.json` is Claude's whole global config, written from 158 `saveGlobalConfig` call sites. Routing it there turns a three-trigger feature into an API call every ten seconds while Claude is in use. The sync can afford it where the quota cannot: a pass that finds nothing costs one credential read and writes nothing.
- **It MAY drive a local CLEAR, and that is the one carve-out (LOCKED).** `oauthAccount` disappearing is this file's one authoritative meaning — Claude's own record of who is signed in, removed on logout and written back on login — and it is the only trace a logout is GUARANTEED to leave: the Keychain emits nothing, the locks around the credential write are version-specific names lifted from a shipped binary, and **the credential entry can SURVIVE a logout**, rebuilt around `coworkRemoteDevice`. `watcher::route_identity_change` reads that one key and calls `invalidate_quota`. The carve-out holds only while it stays free: **read that key and nothing else, and never turn this route into a fetch.** The reverse is deliberately absent — a login must not restore what this cleared, since the account signing in may be a different one, so coming back is a real fetch's job.
- The two signals BESIDE the config dir (`<config-dir>.lock`, `.claude.json`) sit one level ABOVE the watched tree and a lock exists only while held, so their PARENT is watched non-recursively with a name filter. That parent is `$HOME` in the default layout, which churns constantly, so it must also join the rescan gate's ignore list. **The two INSIDE the dir need none of that** — the recursive static watch already covers them — and they must NOT join `claude_signal_paths`: registering the config dir a second time makes notify's two owners of one path fight, downgrading the recursive watch that sessions, memory and skills rely on.
...
- **Claude's FOUR credential signal files (recovered from the binary, v2.1.270).** The Keychain emits nothing, so three are locks `proper-lockfile` mkdirs. Inside the config dir: **`.storage-write.lock`**, held for every secure-storage write, and **`.oauth_refresh.lock`**, the current refresh lock. Beside it: **`<config-dir>.lock`**, now only the LEGACY half of that refresh lock but still taken, and **`.claude.json`**, which is not a credential and stays off this route — it carries the LOGOUT instead, see the identity carve-out above.
- **A logout does NOT reliably delete the credential (LOCKED — measured).** `performLogout` deletes the secure-storage document and then WRITES IT BACK carrying only `coworkRemoteDevice` whenever the account has a device identity: `['claudeAiOauth','coworkRemoteDevice']` becomes `['coworkRemoteDevice']`. So **every signed-out shape must map to `not_found`** — a parsed document with no OAuth entry, an explicit null, and an OAuth block whose `accessToken` is blank (Claude's own `grn()` writes that when the IdP rejects a refresh token). `not_found` is the ONLY status `refresh_quota` clears on; reported as `ParseError` the staleness is PERMANENT, since every later fetch parses the same document and reaches the same verdict. A genuinely torn file stays `ParseError` — it says nothing about who is signed in.
- **`.storage-write.lock` is NOT Claude-login-exclusive and cannot be made so** — it guards the whole secrets document (`designOauth`, `enterpriseGateway`, `trustedDeviceToken`, `coworkRemoteDevice`, `mcpXaaIdp`, `pluginSecrets`, `gatewayTrust`, `mcpDiscoveryCacheKey`) and its name carries no payload to say which one moved. **So the route is GATED on `claude_auth::oauth_credential_changed()`**, which fingerprints the `claudeAiOauth` subtree ALONE: a write to any other secret stops there having cost one credential read, with no fetch, no tray repaint and no account-store pass. Asking CONSUMES the stamp, so it must keep exactly ONE caller.
- **That gate reads UNCACHED, and that is what makes the refresh CORRECT (LOCKED).** Claude's Keychain read is 30s-cached, and a logout answered from that cache queries with the credential just deleted — whose access token is still valid server-side, so the query SUCCEEDS and re-stamps stale numbers as fresh. Not a corner case: `read_credentials_uncached` RE-PRIMES the cache, so any read seconds earlier leaves it live. The re-prime is also why a real change costs nothing extra.
...
- **An EMPTY account-row cache is NOT a reason to rebuild (LOCKED).** A user with no saved login has a menu that legitimately carries no account rows, and the tray CLICK runs this refresh — so treating "no rows" as "cache unusable" put a `set_menu` on every click and the menu dismissed itself the instant it opened, for exactly the users with nothing saved. The CLIs to compare come from the SUBMENU list (`CLI_ROWS`), never from the rows themselves; that is also the only way to see the FIRST account saved under a CLI that had none.
