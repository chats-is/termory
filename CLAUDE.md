...
Current Tauri IPC commands (54 registered in `generate_handler!`), called from the frontend with `invoke(...)`:
...
Only **installed** CLIs get a submenu — `build_menu` runs `detect_installed_clis()` (the same probe as the Providers page's `detect_clis`) and skips any CLI not found, so the tray lists only what you can actually switch. **Install/uninstall auto-refresh:** `static INSTALLED` caches the installed map the VISIBLE menu reflects — stored only AFTER a successful `set_menu` (`do_rebuild_menu_with`) / tray `install()`, never inside `build_menu` (an early store would let a failed rebuild claim the new set and permanently kill the staleness check; on failure the cache keeps the old set so the next refresh retries). `rebuild_if_installed_stale` (main-thread, the single owner of the compare→rebuild step) is fed by three probe paths, each probing on ITS OWN thread and passing the map along so nothing re-probes on main: the watcher's install-detection branch (the same one that emits `cli-install-changed`) calls `tray::refresh_installed`, which RETURNS the probed map so the same burst's rescan reuses it via `refresh_recent_with` (one PATH probe per burst, not two); the `detect_clis` IPC (Providers-page Recheck) hands its result to `tray::refresh_installed_with`; and the shared `commit_recent` path (fed by `refresh_recent` + the tray-click `refresh_work_status`) does the same compare, so an install missed by the watcher self-heals on the next menu open / scan. A stale installed set forces a FULL rebuild (not the recent-region splice — the splice can't refresh the per-CLI submenus; this is the documented exception to "never dismiss the menu" above). **Claude Desktop is covered too:** its install marker is the config dir itself (`claude_desktop::is_installed`), which can't be watched before it exists — so the watcher watches the PARENT dir(s) (`claude_desktop::install_watch_parents()`, derived from the same `config_parent_dir()` helper `current_paths` uses: `~/Library/Application Support`; on Windows the candidate parents incl. the MSIX `Packages\` dir, non-recursive) and `event_touches_claude_desktop` (watcher.rs) filters events to direct children matching `claude_desktop::is_install_marker_name` (the `Claude*` rule, owned next to the Windows dir resolution so it can't drift) before routing them into the same install-changed branch. Those parent dirs are busy shared dirs, so their direct-child events are ALSO excluded from the session-rescan gate (`event_has_relevant_path` takes the parents as an ignore list) — otherwise every other app's activity there would trigger a full `scan_sessions`. **Claude Desktop** is included in the per-CLI provider-switch loop (its own `installed` gate via `claude_desktop::is_installed`, `cli_label`/`cli_key` arms) so it gets a switch submenu when installed, but it is **excluded from the terminal flows** (recent-session resume + "New Session"): those filter on `CliApp::is_cli()`, which is false for the GUI app. It has no quota row (`quota::SUPPORTED` excludes it).
