...
Current Tauri IPC commands (20), called from the frontend with `invoke(...)`:
...
- `read_app_providers` / `write_app_providers` — `~/.termory/providers.json` `providers` array (per-CLI provider library, contains API keys)
- `read_app_gateways` / `write_app_gateways` — `~/.termory/providers.json` `gateways` array (gateway entries with `bindings`; same file, separate array)
...
- `~/.termory/providers.json` — provider library. On disk it's a versioned envelope `{ "version": N, "providers": Provider[], "gateways": Gateway[] }` (`PROVIDERS_SCHEMA_VERSION = 2` in `config.rs`); the wrap/unwrap lives entirely in `config::{read,write}_{providers,gateways}`, so every caller (frontend `config.ts`, `tray.rs`) only ever sees the bare arrays. `read_providers_envelope` reads the object once so `write_providers`/`write_gateways` each preserve the OTHER array. A bare-array file (older dev build / hand-edit) still reads (wrapped as `providers`). The two arrays are kept SEPARATE so the strongly-typed `Vec<Provider>` parse (tray, IPC) never trips over gateway-shaped entries (a gateway has no `app` field). Contains API keys, `0600`.
...
- **Known gaps (not yet done):** tray menu doesn't surface gateway bindings; `detect_gateway_apis` is best-effort (a permissive gateway can answer both OpenAI and Anthropic on `/v1/models`).
