...
- `~/.termory/providers.json` — provider library. On disk it's a versioned envelope `{ "version": N, "providers": Provider[] }` (`PROVIDERS_SCHEMA_VERSION` in `config.rs`); the wrap/unwrap lives entirely in `config::{read,write}_providers`, so every caller (frontend `config.ts`, `tray.rs`) only ever sees the bare `Provider[]`. A bare-array file (older dev build / hand-edit) still reads. Contains API keys, `0600`.
