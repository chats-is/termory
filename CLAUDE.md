...
- **Claude's FOUR credential signal files (recovered from the binary, v2.1.270).** The Keychain emits nothing, so each is a lock `proper-lockfile` mkdirs. Inside the config dir: **`.storage-write.lock`**, held for every secure-storage write — so it is what a **LOGOUT** moves (`performLogout` → `secureStorage.delete()`) — and **`.oauth_refresh.lock`**, the current refresh lock. Beside it: **`<config-dir>.lock`**, now only the LEGACY half of that refresh lock but still taken, and **`.claude.json`**, which is not a credential and stays off this route.
