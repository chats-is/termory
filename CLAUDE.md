...
- Gemini CLI
- OpenCode
- Grok Build (xAI — sessions + skills + provider switching; no memory scan yet, and no official-quota support)
...
Current Tauri IPC commands (55 registered in `generate_handler!`), called from the frontend with `invoke(...)`:
...
- Grok Build (xAI)
  - Grok Build is NOT open source — the on-disk layout itself is the verified reference (surveyed on real data, grok 0.2.93).
  - List: `$GROK_HOME`-aware (`providers::grok_home_dir`, default `~/.grok`) — `<grok-home>/sessions/<percent-encoded-cwd>/<uuid>/summary.json` — read VERBATIM (`info.id`, `info.cwd`, `session_summary` title, `created_at`/`updated_at`, `num_chat_messages`, `current_model_id`); a session dir without `updates.jsonl` is skipped. Sibling noise skipped: `session_search.sqlite`, per-cwd `prompt_history.jsonl`, `.lock` files.
  - Messages: `updates.jsonl` — an ACP (Agent Client Protocol) `session/update` stream. `parse_grok_session`: streaming `user/agent_message_chunk` + `agent_thought_chunk` events merge consecutive same-type chunks into one message (thought → unified `> *…*` blockquote); `tool_call` opens a `**{Label}**(arg)` card using Grok's OWN display label from `_meta["x.ai/tool"].label` (JSON-pointer `~1` escape for the literal `x.ai/tool` key; observed labels: read_file→Read, grep→Search, run_terminal_command→Run Command, search_replace→Edit, list_dir→List Files, todo_write→Plan) and the tool-specific rawInput arg (`grok_tool_arg`); the terminal `tool_call_update` (`completed`/`failed`) finishes the card with `⏺`/`✗` + a 4-backtick output fence (`grok_output_text` unwraps the `{type, <Variant>: {content}}` envelope); `plan` → GFM task list; intermediate tool updates / `retry_state` / `turn_completed` are transport bookkeeping (no content) and are skipped. Resume: `grok --resume <id>` (verified against `grok --help`), wired in terminal.rs + `resumeCommandFor`.
  - Tokens/daily_tokens: not extracted yet (`_meta.totalTokens` exists on events — a future enhancement).
...
| Tool-neutral | `~/.agents/skills/` (`Storage.getUserAgentSkillsDir`) | `<cwd>/.agents/skills/` | global: `codex,gemini,opencode,grok` (Grok Build documents only the USER-level dir); project: `codex,gemini,opencode` |
...
- **Cross-platform is a hard requirement (macOS / Linux / Windows).** The release CI builds all four targets (macOS arm64 + x64, Linux, Windows) on every `v*` tag — keep it green, and don't let a single-platform API leak into a shared path. Concretely:
  ...
  - Backend tests use a `HOME` override (`testutils::override_home`, honored by `crate::home_dir()`'s test-build env seam) and run on ubuntu + macos + **windows** in CI (test.yml). **Known Windows test gap:** the four `migrate_claude_*` tests and the absolute-path `decode_claude_project_slug` assert are `#[cfg(unix)]`-gated — their fixtures are unix-path-shaped (Windows `fs::canonicalize` adds the `\\?\` prefix and JSON escapes the backslashes, so the substring asserts can't hold verbatim); Windows-shaped migrate coverage is an open follow-up, meaning Claude migrate on Windows has ZERO test coverage despite green CI. Green tests ≠ full runtime verification; call out anything that needs real-device testing.
...
- **Backend** (`sessions.rs`, `*_in` inner fns for env-free tests): `scan_project_folders` / `claude_project_folders_in` / `gemini_project_folders_in` / `add_record_projects`; `migrate_claude_{project,session,memory}_in`; `migrate_codex_threads_in` (+ public `migrate_codex_{session,project}`, helpers `rewrite_rollout_cwd` / `codex_canonical_cwd`); `delete_claude_{project,session,memory}_in`; `delete_gemini_{project,record}_in`; helpers `is_safe_rel`, `gemini_project_dirs`, `resolve_new_claude_dir`, `rewrite_jsonl_cwd`, `copy_tree_rewrite_cwd`. IPC wrappers in `lib.rs`.
- **Frontend:** `src/lib/migrate.ts` (`runClaudeMigration`, `runCodexMigration`, `runRecordDelete` — confirm → invoke → toast → success-only local callback); `src/lib/records.ts` (`projectKey`, `projectDirOf`, `recordUnderFolder`, `remappedPath`, `reconcileTombstones`, `withProject`); `recordRel` in `session-utils.ts`; `App.tsx` (`projects` state, `applyScanResult(ScanResult)`, `sourceGroups` rendered from `projects`, `removeRecordsLocally` / `remapRecordsLocally` / `projectRecordMatch` / `projectDataMatch`); per-row menu in `ListItemMenu.tsx`. i18n keys under `menu.migrate*` / `menu.delete*`.

**Tests:** `sessions::tests::{claude_project_folders_in, gemini_project_folders_in, add_record_projects, is_safe_rel, migrate_claude_*, migrate_codex_*, rewrite_rollout_cwd_*, codex_canonical_cwd_*, delete_claude_*, delete_gemini_*}` + `records.test.ts` + `session-utils.test.ts` (`recordRel`) + `ListItemMenu.test.tsx` (per-entry wiring incl. Codex migrate, cancel paths). Whole-project migration was verified end-to-end on real copilot.is → chats.is data (39/39 files moved, `cwd` rewritten, old dir removed).

**Known gaps:** Gemini migrate and OpenCode **migrate** are NOT implemented yet. **Codex migrate IS done** (session/project — metadata rewrite of rollout `payload.cwd` + `threads.cwd`; see "Migrate (Claude + Codex)" above). Codex/OpenCode **delete** IS done (session/project for both, + Codex memory; real delete, sqlite rows by id/cwd + Codex rollout files — see the `delete_codex_*` / `delete_opencode_*` IPC bullets above). (OpenCode empty-project enumeration via the `project` table IS done — `opencode_empty_projects`.)
...
- ~~Frontend test baseline~~ — done. 336 Vitest tests across 27 files.
...
- **Remaining migrate/delete + project gaps for the other CLIs** — see "First-class projects, migration & deletion". **DONE:** delete (session/memory/project) for Claude + Gemini; delete for Codex (session/project/memory) + OpenCode (session/project) — see ② below; migrate (project/session/memory) for Claude; **migrate (session/project) for Codex** (metadata rewrite — see "Migrate (Claude + Codex)"); OpenCode empty-project enumeration. **REMAINING:** Gemini migrate + OpenCode **migrate** (schemas below were already inspected on real data, 2026-06; findings recorded so the next pass can start from the design, not discovery):

  **① Gemini migrate** (file-based, medium effort — the project-hash derivation is the only real risk):
  - Sessions live under a per-project **hash dir** `~/.gemini/tmp/<projectHash>/` with a `.project_root` marker holding the cwd; session JSONLs carry `projectHash` + `sessionId`.
  - Steps: (1) compute Gemini's project-id hash for the NEW path — find `getProjectHash` / `projectIdentifier` in `gemini-cli` (`packages/core/src/config/storage.ts`); it's a hash of the canonical path. (2) move the `chats/` (+ `memory/`) subtree into the new hash dir. (3) rewrite `.project_root` to the new path (and the `projectHash` field inside session records if persisted there).
  - Mirror the Claude pattern: `migrate_gemini_{project,session,memory}_in(tmp, project, rel, new_path, delete_old)` env-free fns + IPC wrappers + frontend via the `runClaudeMigration`-style flow (generalize it to take the command set). Tests on the `_in` fns.

  **② Codex / OpenCode delete + migrate** (SQLite — higher risk; **writing a live CLI DB is an explicit exception to the "read in place" rule, so gate it hard**):
  - Rows, not files. **Codex** `~/.codex/state_5.sqlite` table `threads`: PK `id`, `cwd`, `archived` (0/1) + `archived_at`, `rollout_path` (→ the JSONL on disk). List filter is `archived = 0`. **OpenCode** `~/.local/share/opencode/opencode.db` table `session`: PK `id`, `project_id` FK→`project` `ON DELETE CASCADE`, `directory`, `time_archived`; `message` FK→`session` CASCADE; `part` FK→`message` CASCADE. List filter is `time_archived IS NULL`.
  - **DELETE — DONE (hard delete, by user decision).** Archive-mode (`archived = 1` / `time_archived`) was considered but the user chose REAL deletion to stay consistent with Claude/Gemini (all four = permanent delete). Implemented as `delete_codex_*` / `delete_opencode_*` — see the IPC bullets in "First-class projects, migration & deletion" for the exact mechanism (Codex: row + rollout file, because the row alone is rebuilt by backfill; OpenCode: cascade DELETE + event-log clear). Codex project delete is "full" (includes archived threads). OpenCode has no deletable memory.
  - **MIGRATE — Codex DONE, OpenCode still TODO.** Codex: `migrate_codex_*` rewrites the rollout `payload.cwd` (authoritative — the table is a backfill cache) + `UPDATE threads.cwd` (see "Migrate (Claude + Codex)"). OpenCode TODO: `UPDATE session.directory` (+ maybe `project.worktree` / `project_id`) — but OpenCode's `session` table is a durable store (not a backfill cache), so a direct `UPDATE` sticks without a file rewrite. Lower value either way: resume-by-id already works across renames (global DB keyed by id), so migrate here is a **regrouping** convenience, not a "records lost" fix.
  - **Safety pattern (used by the delete impl, reuse for migrate):** open read-write with a 1.5s `busy_timeout`; if the DB is locked → "database is locked — quit the running {app}" (`map_db_locked`) — the delete confirm's `menu.exitCliHint` ("quit the running {app} first") already primes the user. No DB backup is taken (cache self-heals / direct deletes are intentional). Env-free `*_in(db, …)` fns + IPC + a confirm flow that sends id / `{ project }` / `{ rel }` (never a raw path).
