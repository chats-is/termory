...
- **Providers** — Keep named API profiles for each CLI and switch the active one with a click, or from the menu bar. See your official-plan usage — rate-limit windows plus pay-as-you-go **Usage credits** — right on the card, and a badge when a CLI has a newer version — click it to upgrade the CLI in place. Codex supports multiple saved accounts — add, switch, and re-authenticate without losing session history, and your saved logins are one click away in the menu bar too.
