...
- **Providers** — Keep named API profiles for each CLI and switch the active one with a click, or from the menu bar. See your official-plan usage right on the card — rate-limit windows, pay-as-you-go **Usage credits**, and any prepaid **Balance**, each shown only when your account actually has it — and a badge when a CLI has a newer version — click it to upgrade the CLI in place. Codex, Claude Code and Grok Build support multiple saved accounts — add, switch, and re-authenticate without losing session history, and your saved logins are one click away in the menu bar too. Saved accounts keep themselves current: Termory follows the live login's own credential, so a token refresh, a re-login you did in a terminal, or a plan change all show up on their own.
...
| Tool | Sessions | Memory | Skills | Provider switch | Multi-account |
|------|:--------:|:------:|:------:|:---------------:|:-------------:|
| Codex | ✅ | ✅ | ✅ | ✅ | ✅ |
| Claude Code | ✅ | ✅ | ✅ | ✅ | ✅ |
| Gemini CLI | ✅ | ✅ | ✅ | ✅ | — |
...
Grok Build (xAI) covers sessions, memory, skills, provider switching (via xAI's official custom-model mechanism — `[model."<id>"]` entries in `~/.grok/config.toml`), and multiple official logins. Provider switching still never touches your auth.x.ai login; only account switching writes it, and only when you ask for it. Claude Desktop is the GUI app (no terminal history), so it's **provider-switching only** — point it at a third-party Anthropic-compatible provider and back to Official. macOS and Windows only.
