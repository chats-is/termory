...
在 **Codex** 标签页,Official 卡片还能管理多个 ChatGPT 登录。**保存当前**把当前登录存为快照;**添加账号**在浏览器发起一次全新的 `codex login`(可取消——取消或失败时自动恢复你原来的登录)。只装桌面应用也能用——Termory 会调用应用内自带的 CLI 完成登录。每条已保存的账号会显示邮箱、订阅计划和令牌最近刷新时间。**切换**恢复某个快照——Termory 会先刷新它的令牌,登录已失效的账号会标记为**重新登录**,而不是写入一份坏凭据。快照存放在 `~/.termory/accounts.json`(仅所有者可读写);Codex 自己的 `auth.json` 只在切换或添加时才会被写入。

账号与服务商互不影响,尽管 Codex 把两者存在同一个文件里:切换账号只改变当前生效的 ChatGPT 登录,不会动你选中的服务商及其 API Key。保存账号只记录登录本身——第三方 Key 不会被复制进快照,因此日后恢复某个快照也不可能把别处的 Key 带回来。无论从应用内还是菜单栏切换,行为都一致。
...
- **Codex 账号** — 已保存的 ChatGPT 登录就排在 Codex 子菜单的 **Official** 下方,点一下即可切换。当前登录带勾选;令牌已失效的会显示 **⚠** 且不可点击,需先到供应商页重新登录。从这里切换会先把当前登录快照保存,所以即便你从没点过**保存当前**也不会丢。
