...
| 5 | **[统计 Stats](#5-stats统计)** | 在任意日期范围内的 Tokens、消息、模型与活跃度热力图。 |
...
- **KPI 条** — 会话、消息、Tokens、模型、项目。悬停 **Tokens** 看 输入 / 输出 / 推理 / 缓存 / 总计 拆分;悬停**模型**看按模型的 token 用量。
- **每日 Tokens** — 含 输入 / 输出 / 缓存 / 推理 四条线的趋势图;悬停看当天拆分。
- **每日活跃度** — 24 小时 × 日期 的热力图;格子深浅由消息数与 token 共同决定。悬停某格看该小时的 会话 / 消息 / token 及按模型用量。
