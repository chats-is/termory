...
两点作用范围说明:切换 **Codex** 会同时作用于 Codex CLI 和 Codex 应用(两者共用同一份配置与登录);而 **Claude Desktop** 是独立的标签页、独立的供应商库——与 Claude Code 完全分开管理。
...
在 **Codex** 标签页,Official 卡片还能管理多个 ChatGPT 登录。**保存当前**把当前登录存为快照;**添加账号**在浏览器发起一次全新的 `codex login`(可取消——取消或失败时自动恢复你原来的登录)。每条已保存的账号会显示邮箱、订阅计划和令牌最近刷新时间。**切换**恢复某个快照——Termory 会先刷新它的令牌,登录已失效的账号会标记为**重新登录**,而不是写入一份坏凭据。快照存放在 `~/.termory/accounts.json`(仅所有者可读写);Codex 自己的 `auth.json` 只在切换或添加时才会被写入。**Claude Code** 和 **Gemini** 的卡片只展示当前登录的官方账号——保存与切换目前仅支持 Codex。
