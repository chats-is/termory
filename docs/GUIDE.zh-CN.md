...
### 官方账号(Codex 与 Claude Code 多账号)

在 **Codex** 和 **Claude Code** 标签页,Official 卡片还能管理多个官方登录。**保存当前**把当前登录存为快照;**添加账号**在浏览器发起一次全新登录(`codex login` / `claude auth login`,可取消——取消或失败时自动恢复你原来的登录)。Codex 只装桌面应用也能用——Termory 会调用应用内自带的 CLI 完成登录。每条已保存的账号会显示邮箱、订阅计划和令牌最近刷新时间。**切换**恢复某个快照——Termory 会先刷新它的令牌,登录已失效的账号会标记为**重新登录**,而不是写入一份坏凭据。快照存放在 `~/.termory/accounts.json`(仅所有者可读写);CLI 自己的凭据存储只在切换或添加时才会被写入。
