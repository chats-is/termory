...
| 5 | **[统计 Stats](#5-stats统计)** | 总览 KPI + 日历热力图,以及按模型的 token 用量(全部 / 30天 / 7天)。 |
...
**怎么用:** 选范围——**全部 / 30天 / 7天**,可按来源过滤。两个视图:

**总览** ——

- **8 张 KPI 卡** — 会话、消息、总 Tokens、活跃天数、当前连续、最长连续、高峰时段、常用模型。
- **日历热力图** — 每天一个方格(GitHub 风格的周列);深浅由消息数与 token 共同决定,悬停看当天日期、消息、tokens。

**模型** ——

- 按天按模型拆分的 tokens **堆叠柱状图**(靠前的模型各有配色,其余归入"其他")。
- 一份**图例**,列出每个模型的输入/输出 tokens 及占窗口用量的比例。

> 模型归因是按会话粒度的(每个会话归到它记录的那个模型);没有记录模型的会话在 KPI/图例中隐藏,但仍计入总量。
