...
| # | 入口 | 作用 |
|---|------|------|
| 1 | **[供应商 Providers](#1-providers供应商)** | 管理每个 CLI 的 API 供应商并切换当前激活项;管理 AI Gateway;查看官方额度;保存与切换 Codex / Claude Code 账号。 |
...
Grok Build 的登录存在一个普通文件里,但 xAI 每次使用刷新令牌都会作废它——所以存放了一段时间的快照,里面那份可能已经失效。Termory 在写入任何东西之前会先向 xAI 核验:如果这个快照已经失效,你会当场收到报错、该行标记为**重新登录**、当前登录毫发无损,而不是切换看起来成功、几小时后把你登出。如果只是连不上 xAI,则按快照原样恢复,由 grok 下次启动时自行刷新。一个注意事项:如果你是在终端里换的 grok 账号而不是在 Termory 里换的,Termory 为你离开的那个账号保存的快照可能会过期,再切回去时会要求重新登录。
...
**Gemini** 和 **Grok Build** 的卡片只展示当前登录的官方账号——保存与切换支持 Codex 与 Claude Code。
...
- **Codex / Claude Code 账号** — 已保存的官方登录就排在对应 CLI 子菜单的 **Official** 下方,点一下即可切换。当前登录带勾选;令牌已失效的会显示 **⚠** 且不可点击,需先到供应商页重新登录。从这里切换会先把当前登录快照保存,所以即便你从没点过**保存当前**也不会丢。
