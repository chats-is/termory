...
| **Terminal** | Which terminal opens when you resume a session. Only what's found on this machine is listed, and **Default** follows your OS. On Windows the list is *shells* instead (PowerShell 7 / PowerShell / CMD / Git Bash) — which terminal app hosts them stays whatever you set in Windows itself. |
